...
start_idx = content.find('    def _embed_browser_window(self):')
end_idx = content.find('    def _lock_browser_position(self, hwnd, parent_hwnd, w, h):')
...
'''
    ...
    with open('cdp_worker.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('Successfully replaced _embed_browser_window')
else:
